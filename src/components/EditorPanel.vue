...
import { resolveNodeById } from "../editor/core/flow-serialization";
...
function onSelect(e: EditorEvent) {
  ...
  selectedConnectorLabel.value = Boolean(selectedElement.value && getConnectorLabelTarget(selectedElement.value));
...
function resolveSelectedParts() {
  ...
  if (el.tag === "Group" && children?.length) {
    ...
    selectedText.value = (children.find((child) => child instanceof Text) as Text | undefined) || null;
    return;
  }
...
function updateState() {
  ...
  if (shape) {
    ...
    strokeWidth.value = typeof shape.strokeWidth === "number" ? shape.strokeWidth : strokeWidth.value;
...
function updateConnectorSide(which: "from" | "to", value: ConnectorSide | "auto") {
  ...
  const connector = selectedConnector.value;
  if (!connector || !props.editor) return;

  const state = connector.getState() as ConnectorState;
  const nextState = {
    ...state,
    opt1: { ...(state.opt1 || {}), side: fromSide.value, percent: 0.5 },
    opt2: { ...(state.opt2 || {}), side: toSide.value, percent: 0.5 },
  } as ConnectorState;

  try {
    connector.setState(nextState, (id: string | number) => resolveNodeById(props.editor!.app, id));
    syncAfterChange();
  } catch (error) {
...
function applyToSelectedConnectors(updater: (connector: Connector) => void) {
  const connectors = selectedElements.value.filter((el): el is Connector => el instanceof Connector);
  ...
  <Transition name="slide-fade">
    ...
    >
      <div class="flex justify-between items-center p-2 bg-base-200/50 cursor-move select-none" @mousedown="startDrag">
        ...
        <button class="btn btn-ghost btn-xs btn-square" @click.stop="toggleCollapse(isDragging)" @mousedown.stop :title="isCollapsed ? '展开属性' : '折叠属性'">
      ...
      <div class="card-body p-3 pt-2 max-h-[70vh] overflow-y-auto" v-show="!isCollapsed">
        ...
        <div class="mb-1">
          ...
          <div class="grid grid-cols-2 gap-1">
            ...
            >
              <span class="h-3 w-3 rounded border" :style="{ backgroundColor: preset.fill, borderColor: preset.stroke }"></span>
              {{ preset.label }}
            </button>
          </div>
        </div>
        ...
        <div v-if="!selectedConnector && !isMultiSelection" class="grid grid-cols-2 gap-2">
          <label class="form-control w-full">
            ...
            <input type="number" min="1" :value="width" @input="(e) => updateWidth(Number((e.target as HTMLInputElement).value))" class="input input-bordered input-xs w-full" />
          </label>
          <label class="form-control w-full">
            <div class="label p-1"><span class="label-text text-xs">高</span></div>
            <input type="number" min="1" :value="height" @input="(e) => updateHeight(Number((e.target as HTMLInputElement).value))" class="input input-bordered input-xs w-full" />
          </label>
        </div>
        ...
        <div v-if="selectedShape && !selectedConnector" class="grid grid-cols-2 gap-2">
          <label class="form-control">
            ...
            <input type="color" :value="fill" @input="(e) => updateFill((e.target as HTMLInputElement).value)" class="h-7 w-full cursor-pointer rounded border border-base-300" />
          </label>
          <label class="form-control">
            <div class="label p-1"><span class="label-text text-xs">边框</span></div>
            <input type="color" :value="stroke" @input="(e) => updateStroke((e.target as HTMLInputElement).value)" class="h-7 w-full cursor-pointer rounded border border-base-300" />
          </label>
        </div>
        ...
        <template v-if="selectedConnector">
          ...
          <label class="form-control w-full">
            ...
            <select class="select select-bordered select-xs w-full" :value="routeType" @change="(e) => updateRouteType((e.target as HTMLSelectElement).value as ConnectorRouteType)">
          ...
          <label class="form-control w-full">
            ...
            <select class="select select-bordered select-xs w-full" :value="arrowMode" @change="(e) => updateArrowMode((e.target as HTMLSelectElement).value as 'none' | 'end' | 'both')">
          ...
          <label class="label cursor-pointer justify-start gap-2 px-1">
            <input type="checkbox" class="toggle toggle-xs" :checked="lineDashed" @change="(e) => updateLineDashed((e.target as HTMLInputElement).checked)" />
          ...
          <label class="form-control w-full">
            ...
            <input type="range" min="0" max="40" step="1" :value="lineCornerRadius" @input="(e) => updateLineCornerRadius(Number((e.target as HTMLInputElement).value))" class="range range-xs" />
          </label>

          <div class="grid grid-cols-2 gap-2">
            <label class="form-control w-full">
              <div class="label p-1"><span class="label-text text-xs">起点锚点</span></div>
              <select class="select select-bordered select-xs w-full" :value="fromSide" @change="(e) => updateConnectorSide('from', (e.target as HTMLSelectElement).value as ConnectorSide | 'auto')">
            ...
            <label class="form-control w-full">
              ...
              <select class="select select-bordered select-xs w-full" :value="toSide" @change="(e) => updateConnectorSide('to', (e.target as HTMLSelectElement).value as ConnectorSide | 'auto')">
                <option value="auto">自动</option>
                <option value="top">上</option>
                <option value="right">右</option>
                <option value="bottom">下</option>
        ...
        <template v-if="selectedText">
          ...
          <label v-if="!isMultiSelection" class="form-control w-full">
            ...
            <textarea :value="textContent" @input="(e) => updateText((e.target as HTMLTextAreaElement).value)" class="textarea textarea-bordered textarea-xs w-full min-h-16 resize-y"></textarea>
          </label>
          ...
          <div class="grid grid-cols-2 gap-2">
            <label class="form-control">
              ...
              <input type="color" :value="textColor" @input="(e) => updateTextColor((e.target as HTMLInputElement).value)" class="h-7 w-full cursor-pointer rounded border border-base-300" />
            </label>
            <label class="form-control">
              <div class="label p-1">
              ...
              <input type="range" min="8" max="72" step="1" :value="fontSize" @input="(e) => updateFontSize(Number((e.target as HTMLInputElement).value))" class="range range-xs mt-2" />
            </label>
          </div>
        </template>
      </div>
