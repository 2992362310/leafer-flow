...
function smartGroupSelection(list: IUI[]): IUI[] {
  ...
  parentGroupMap.forEach((selectedChildren, parentGroup) => {
    ...
    // 检查是否选中了所有子元素
    const allSelected =
      allChildren.length === selectedChildren.length &&
...
export function doGroup(editor: Editor): { success: boolean; message: string } {
  ...
  if (list.length < 2) {
    // 如果经过 smartGroupSelection 后只剩一个元素，说明选中了一个已有 Group 的全部子元素
    // 该 Group 已经存在，无需再次编组
    if (list.length === 1 && list[0] instanceof Group) {
      return { success: false, message: "这些元素已经在一个组合中，无需重复编组" };
    }
